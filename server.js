...
const FONTAWESOME_KIT_URL = 'https://kit.fontawesome.com/c2cba4c74e.js';
...
// ─── Math protection ───
// Marked.js mangles math with multiple underscores or asterisks (e.g. $x_1 + y_2$
// becomes nonsense). We extract math regions before marked runs, replace with
// placeholders, then restore the math after. MathJax handles rendering at view time.
//
// Recognised delimiters:
//   $$...$$  display math (greedy across multiple lines)
//   \[...\]  display math
//   $...$    inline math (must not start with whitespace or digit, to avoid prices)
//   \(...\)  inline math
function extractMath(md) {
  ...
  // Display math first (so $$...$$ isn't matched as two $...$)
  let out = md;
  ...
  // $...$  — only match when start char isn't whitespace/digit (avoids "$5 a litre")
  // and there's no $ in between. Single-line only.
  out = out.replace(/\$([^\s\d$][^$\n]*?)\$/g, (_, math) => {
    const ph = placeholder();
    placeholders.push({ ph, replacement: '$' + math + '$' });
    return ph;
...
function restoreMath(html, placeholders) {
  ...
  for (const { ph, replacement } of placeholders) {
    // Placeholder might end up inside a <p> tag, etc. Just replace the token.
    out = out.split(ph).join(replacement);
  }
  return out;
}

// Render markdown safely. Returns sanitised HTML with math preserved for MathJax.
...
// Detect whether a note contains math. If so, we load MathJax on the interstitial;
// otherwise we skip the script entirely to keep the page light.
...
function renderInterstitial({ code, kind, target, title, noteHtml, noteExcerpt, noteRaw, fileSize }) {
  ...
  // Build optional <head> additions
  const mathjaxConfig = needsMath ? `
<script>
  window.MathJax = {
    tex: {
      inlineMath: [['$', '$'], ['\\\\(', '\\\\)']],
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  main {
    ...
    padding: 40px 0; animation: fadein 0.5s ease-out;
  }
  @keyframes fadein {
  ...
  /* MathJax SVG output: ensure it inherits ink colour and sits well in line */
